...
import click
import frappe
from renovation_core.install import get_jinja_env
...
def use_wildcard_certificate(bench_path, ret):
  '''
    stored in common_site_config.json as:
      "wildcard": {
      "domain": "*.leam.dev",
      "ssl_certificate": "/path/to/leam.dev.cert",
      "ssl_certificate_key": "/path/to/leam.dev.key"
    }
  '''
  from .common_site_config import get_config
...
def get_error_pages():
  return {
      502: frappe.get_app_path("renovation_core", "install", "templates", "502.html")
  }
